...
# For a 200-member population the search already carries substantial structural
# diversity, so the controller step size is reduced relative to the earlier
# 100 x 100 exploration script to avoid spending evaluator effort on violently
# distorted offspring that mostly relax into failures or duplicates.
STEP_SIZE="${STEP_SIZE:-0.24}"
TEMPERATURE="${TEMPERATURE:-10.0}"

# The staged evaluator now prefers cleaner accepted structures over permissive
# carry-forward behavior. GNORM is still written into run.job for Scott-style
# provenance, but the actual GULP convergence behavior is driven explicitly by
# gtol/gmax/ftol/xtol in Master.gin.
SCOTT_GNORM="${SCOTT_GNORM:-0.10}"
GULP_GTOL="${GULP_GTOL:-${SCOTT_GNORM}}"
GULP_GMAX="${GULP_GMAX:-${SCOTT_GNORM}}"
GULP_FTOL="${GULP_FTOL:-0.002}"
GULP_XTOL="${GULP_XTOL:-0.002}"
GULP_MAXCYC="${GULP_MAXCYC:-350}"
GULP_MINIMIZER="${GULP_MINIMIZER:-lbfgs}"

SYSTEM_LABEL="${SYSTEM_LABEL:-'(MgO)25'}"
RUN_NAME="${RUN_NAME:-mgo25_staged_gulp_100g200p_scientific_${STAMP}}"
...
# Literature-informed staged evaluator contract for (MgO)25.
# Roberts & Johnston (PCCP 2001) and Johnston (Dalton 2003) justify broad
# structural exploration for ionic MgO clusters, while Lazauskas, Sokol and
# Woodley (Nanoscale 2017) emphasize diversity preservation and aggressive
# duplicate removal in Lamarckian cluster GA workflows.
#
# This staged Rust path currently uses the Rust controller's built-in GULP
# operator profile; run.job controls the evaluator contract and provenance.
N_DEF_ENERGY:GULP
...
# The smaller jitter keeps the seed recognisably ionic-rocksalt-like while still
# breaking exact symmetry so duplicate screening is not dominated by one perfect seed.
a = 2.106
species = []
coords = []
for i in range(5):
...
echo
echo "Note: the staged Rust GA currently keeps the built-in GULP operator policy"
echo "      (including the small tournament window). This script tunes the knobs"
echo "      that are actually active today: population, perturbation size, and the"
echo "      evaluator/convergence contract."
echo
